...
"""
THIS IS JUST CHEMPROT COPIED FOR TESTING PURPOSES
"""
import os
...
class TestDataset(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        BigBioConfig(
            ...
            description="test source schema",
            schema="source",
            subset_id="test_full",
        ),
        BigBioConfig(
            name="test_shared_task_eval_source",
            version=SOURCE_VERSION,
            description="test source schema with only the relation types that were used in the shared task evaluation",
            schema="source",
            subset_id="testdata_shared_task_eval",
        ),
        BigBioConfig(
            name="test_bigbio_kb",
            version=BIGBIO_VERSION,
            description="test BigBio schema",
            schema="bigbio_kb",
            subset_id="testdata",
        ),
    ]

    DEFAULT_CONFIG_NAME = "test_full_source"
    ...
    def _generate_examples(
        ...
        if self.config.schema == "source":
            ...
            if self.config.subset_id == "test_full":
                relations = self._get_relations(os.path.join(filepath, relation_file))
            elif self.config.subset_id == "test_shared_task_eval":
